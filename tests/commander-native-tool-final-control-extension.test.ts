/**
 * Hermetic unit tests for the FINAL control extension
 * (`scripts/commander-native-tool-final-control-extension.ts`).
 *
 * The FINAL control proxy is exercised ONLY against a stub `ExtensionAPI`:
 * the full workbench runtime (`extensions/workbench-runtime/index.ts`) is
 ...
 *     reused by identity via `NATIVE_OVERRIDE_NAMES` — never duplicated;
 ...
 *   - property reads/writes land on the original target (the proxy
 *     accumulates no own state);
 *   - only `registerTool` is intercepted — other API methods delegate with
 *     arguments preserved;
 *   - the default export is the FINAL control factory (never invoked by
 *     tests) and is independently labeled from the DEV-pilot approximation.
 */
...
import finalControlExtension, {
	createFinalControlApi,
	FINAL_CONTROL_LABEL,
	FINAL_SUPPRESSED_TOOL_NAMES,
	isFinalControlSuppressedToolName,
} from "../scripts/commander-native-tool-final-control-extension.ts";
...
/** Loose surface view of the FINAL control proxy for non-API stub methods. */
interface FinalControlSurface {
...
test("suppression names are exactly the canonical three, reused by identity not duplicated", () => {
	assert.deepEqual([...NATIVE_OVERRIDE_NAMES], ["read", "grep", "find"]);
	assert.deepEqual([...FINAL_SUPPRESSED_TOOL_NAMES], ["read", "grep", "find"]);
	// identity: the FINAL control list IS the canonical list — never a copy
	assert.equal(FINAL_SUPPRESSED_TOOL_NAMES, NATIVE_OVERRIDE_NAMES);
	for (const name of NATIVE_OVERRIDE_NAMES) {
		assert.equal(isFinalControlSuppressedToolName(name), true);
	}
	...
	for (const name of ["Read", "reader", "find_all", "grep_count", "workbench_read_run", "read2"]) {
		assert.equal(isFinalControlSuppressedToolName(name), false);
	}
});

test("read/grep/find registrations are suppressed; all others delegate unchanged in order", () => {
	const stub = makeStub();
	const controlled = createFinalControlApi(stub);
...
test("non-register methods are bound to the original target (receiver preserved)", async () => {
	const stub = makeStub();
	const controlled = createFinalControlApi(stub);
	const surface = controlled as unknown as FinalControlSurface;
...
test("property reads and writes land on the original target", () => {
	const stub = makeStub();
	const controlled = createFinalControlApi(stub);
	const surface = controlled as unknown as FinalControlSurface;
...
test("default export is the FINAL control factory, independently labeled, never invoked by tests", () => {
	assert.equal(typeof finalControlExtension, "function");
	assert.equal(FINAL_CONTROL_LABEL, "final-arm-control");
	// independent from the DEV-pilot approximation — the DEV-only wrapper is
	// never used as final evidence (documented label of
	// scripts/commander-native-tool-control-extension.ts)
	assert.notEqual(FINAL_CONTROL_LABEL, "dev-pilot-control-approximation");
	assert.ok(FINAL_CONTROL_LABEL.length > 0);
});
