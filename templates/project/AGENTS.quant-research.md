...
- The workbench runs a gate ladder: base gates `b0`-`b6` (project readiness,
  ...
  reproducibility, development safety) for every profile, plus quant
...
- B6 (Development Safety; legacy machine kind `worker-first`) is
  machine-backed: the runtime injects bounded safety facts into every gate run
  (development policy active, no
...
- Use focused tests during development. Run final gates once on a stable
  candidate when research or release risk requires them.
